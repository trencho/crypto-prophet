...
- **CI** (`.github/workflows/ci.yml`): black + ruff + compile-check + pytest on Python 3.14.
- **Security:** a weekly OSV scan (`security-scan.yml`) compiles the fully-pinned tree from both
  manifests and scans that, so nothing is regenerated by hand and the scan always reads the
