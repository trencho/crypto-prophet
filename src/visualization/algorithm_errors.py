...
def draw_errors(cryptocurrency):
    error_types = [
        'Mean Absolute Error',
    ...
    for error_type in error_types:
        ...
        for algorithm in regression_models:
            dataframe_errors = read_csv(
                path.join(RESULTS_ERRORS_PATH, 'data', cryptocurrency['symbol'], algorithm, 'error.csv'))
            dataframe_algorithms = dataframe_algorithms.append(
                [{
                    'algorithm': regression_models[algorithm],
                    cryptocurrency['symbol']: dataframe_errors.iloc[0][error_type]
                }], ignore_index=True).dropna()

        if dataframe_algorithms.empty:
            continue

        dataframe_algorithms.sort_values(by=cryptocurrency['symbol'], ascending=False, inplace=True)
        dataframe_algorithms.reset_index(drop=True, inplace=True)

        fig, ax = plt.subplots(figsize=(16, 10), facecolor='white', dpi=80)
        ax.vlines(x=dataframe_algorithms.index, ymin=0, ymax=dataframe_algorithms[cryptocurrency['symbol']],
                  color='firebrick', alpha=0.7, linewidth=40, label=cryptocurrency["name"])
        ax.legend()

        # Annotate Text
        for i, value in enumerate(dataframe_algorithms[cryptocurrency['symbol']]):
            ax.text(i, value + 0.5, round(value, 1), fontsize=22, horizontalalignment='center')

        # Title, Label, Ticks and Ylim
        ax.set_title(cryptocurrency['symbol'], fontdict={'size': 22})
        ax.set_ylabel(error_type, fontsize=22)
        y_lim = dataframe_algorithms[cryptocurrency['symbol']].max() + 10 - (
                dataframe_algorithms[cryptocurrency['symbol']].max() % 10)
        ...
        file_path = path.join(RESULTS_ERRORS_PATH, 'plots', cryptocurrency['symbol'])
